...
internal sealed class PaddleOcrStdioTransport : IPaddleOcrTransport
{
    ...
    public async Task<string> InvokeAsync(string requestJson, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (response is null)
            {
                ThrowIfWorkerExited("while waiting for response");
                throw new InvalidOperationException(
                    "PaddleOCR worker closed stdout before sending a response. " +
                    "Protocol requires stdout to contain only one line-delimited JSON response per request; " +
                    "send all diagnostics/logging to stderr.");
            }
